...
- **Company Acquisition (prestige)** — once you've built enough *total accrued valuation*, accept an acquisition offer to bank permanent **Equity** (diminishing-returns payout), soft-reset into a fresh company (re-picking a founder; Equity + skill tree persist), and spend Equity in a Path-of-Exile-style **skill tree** (~310 procedurally-laid-out nodes across themed clusters, rendered on its own PixiJS viewport with HTML node previews). Costs escalate per node allocated; the tree is **respeccable** (Exits buy persistent respec points). The full loop — accrual tracking, payout, soft reset, allocation, respec, persistence — is in; the node *effects* are still being wired to the economy (see `docs/acquisition-skill-tree-design.md`).
